...
CONFIG = {
    ...
    "gamma": 0.99,
    "total_eps": 20000,
    "ep_length": 10,  # longer episodes help exploit joint-action observations
    "eval_freq": 400,
    "lr": 0.05,
    "init_epsilon": 0.9,
    "eval_epsilon": 0.05,
...
def train(env, config, output=True):
    ...
    for eps_num in range(config["total_eps"]):
        ...
        if eps_num > 0 and eps_num % config["eval_freq"] == 0:
            mean_return, std_return = iql_eval(env, config, agents.q_tables, output=output)
...
if __name__ == "__main__":
    ...
    # Example payoff matrix (stag hunt)
    stag_hunt = np.array([[[5, 5], [0, 4]],
                              [[4, 0], [3, 3]]])
    env = JAL(stag_hunt, CONFIG["ep_length"])
