...
class MoveChairSimple(gym.Env):
    def __init__(self, ep_length, step_cost=0.05, reward_shaping=False):
        ...
        self.observation_space = gym.spaces.Tuple([
            gym.spaces.Tuple([
                gym.spaces.Discrete(2),  # self_pos: 0, 1
                gym.spaces.Discrete(2),  # self_has_chair: 0, 1
                gym.spaces.Discrete(2),  # opp_pos: 0, 1
                gym.spaces.Discrete(2),  # opp_has_chair: 0, 1
                gym.spaces.Discrete(2),  # chair_pos: 0, 1
        ...
        self._state = np.array([1, 0, 1, 0, 1, 0], dtype=np.int32)
    ...
    def _is_terminal(self):
        ...
        if s[0] == 0 and s[2] == 0 and s[4] == 0 and s[5] == 1:
    ...
    def step(self, actions):
        ...
        for i, a in enumerate(actions):
            ...
            if a == 0:
                if common_state[0 + (2*i)] == 0 and common_state[1 + (2*i)] == 0: 
        ...
        if self.reward_shaping and not done:
            ...
            agents_holding_door = [
                common_state[0] == 0 and common_state[1] == 0 and actions[0] == 0, 
                common_state[2] == 0 and common_state[3] == 0 and actions[1] == 0
            ]
            ...
            for i in range(self.n_agents):
                ...
                if new_has_chair[i]:
                    chair_progress = (new_chair_pos - prev_chair_pos) * 0.1
